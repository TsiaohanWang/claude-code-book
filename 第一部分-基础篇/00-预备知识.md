...
在纯 JavaScript 中，变量的类型在**运行时**（Runtime——程序实际执行的时候，区别于"编译时"即代码被检查和转换的阶段）才确定：
...
`async/await` 是 Promise 的**语法糖**（Syntactic Sugar——不引入新功能，但让代码更易读写的简写语法），让异步代码拥有同步代码的书写体验：
...
Zod 使用**链式 API**（Chaining API——方法调用可以像链条一样串起来，如 `z.string().min(1).max(100)`，每个方法返回对象自身，允许继续调用下一个方法）定义数据的"形状"：
...
- `useEffect` 注册**副作用**（Side Effect——函数除了返回值之外对外部世界造成的影响，如修改文件、发送网络请求、设置定时器），返回清理函数防止内存泄漏
...
WebSocket 是一种在单个 TCP 连接上进行**全双工**（Full Duplex——通信双方可以同时发送和接收数据，如电话）通信的协议——客户端和服务端可以同时向对方发送数据：
