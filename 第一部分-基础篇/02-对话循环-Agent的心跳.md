...
面对这些需求，传统的函数调用模型力不从心。异步生成器提供了完美的答案：它像一个可以随时暂停和恢复的"**协程**"（Coroutine——一种可以暂停和恢复执行的子程序，比线程更轻量），在"**生产者**"（Producer——产生数据的一方，此处指对话循环）和"**消费者**"（Consumer——消费数据的一方，此处指 UI 渲染层）之间建立了一条实时的事件管道。
...
> **设计哲学对比：** 如果使用回调模式，你需要为每种事件类型注册独立的回调函数，代码会变成分散在各处的回调处理逻辑。如果使用 Promise 链，虽然避免了回调地狱，但 Promise 是"一次性的"——它只能 resolve 一次，无法表达持续的事件流。如果使用 **RxJS Observable**（一个用于处理异步数据流的库，功能强大但引入了额外依赖和陡峭的学习曲线），虽然功能强大，但引入了沉重的依赖和陡峭的学习曲线。AsyncGenerator 是"刚刚好"的方案——原生语言支持、零额外依赖、类型安全、天然支持流式和取消。
...
`queryLoop` 是一个 `while(true)` 无限循环。每次迭代代表一次"模型调用 + 工具执行"的完整回合。在循环顶部，函数从状态对象中**解构**（Destructuring——从对象或数组中提取字段赋给独立变量的语法，如 `const { name, age } = person`）出当前迭代所需的变量，包括工具使用上下文、消息列表、自动压缩追踪、恢复计数器等。

状态对象是一个可变状态容器，包含跨迭代传递的全部状态：消息列表、工具上下文、自动压缩追踪、恢复计数器、turn 计数等。每次 `continue` 回到循环顶部时，都会写入一个新的状态对象。

这个设计的关键洞察是：**状态在"读"和"写"之间有明确的分界线。** 在每次迭代的开始，函数通过解构一次性读取所有需要的状态字段（**快照**语义——某一时刻状态的完整副本，后续修改不影响已读取的快照）；在迭代结束时，通过构造新对象一次性写入更新后的状态（**原子更新**语义——要么全部更新成功，要么完全不更新，不存在"改了一半"的中间状态）。这避免了在迭代过程中部分更新导致的不一致问题。
...
| `model_error` | API 调用异常 | Agent 报错并展示错误信息 | 网络或服务端问题的**优雅降级**（Graceful Degradation——系统遇到错误时不完全崩溃，而是退回到一个可用但功能受限的状态） |
...
> **设计洞察：** 十种终止原因的精细划分并非过度设计。在调试 Agent 行为时，准确的终止原因是定位问题的第一线索。如果所有错误都返回一个笼统的"error"，开发者将无从判断是 API 超时、上下文溢出还是用户中断导致了问题。细粒度的终止原因是"**可观测性**"（Observability——从系统外部的输出如日志、指标、追踪来推断系统内部状态的能力）的基础。
...
对话主循环的设计中最值得注意的工程决策之一是**依赖注入**（Dependency Injection，简称 DI）模式。这是一种软件设计技巧：不把函数需要的外部能力（如调用 API、生成随机数）硬编码在函数内部，而是通过参数从外部"注入"进来。好处是测试时可以用假的实现替换真实的实现——比如用一个返回固定结果的假 API 调用替代真实的网络请求，从而在不联网的情况下测试代码逻辑。

系统定义了一个精简的依赖接口，包含四个核心依赖：模型调用函数、轻量压缩函数、自动压缩函数和 UUID 生成器（Universally Unique Identifier，通用唯一标识符，一串几乎不可能重复的随机 ID）。
...
这段注释揭示了一个重要的工程洞察：在没有依赖注入的情况下，测试代码需要通过模块级别的 **spy/mock**（"间谍/模拟"——测试中用来拦截函数调用、记录调用次数或返回预设结果的假对象）来替换外部依赖。这种 spy-per-module 的模式有几个问题：它耦合了测试代码与模块内部结构，当模块重命名或移动时测试需要同步修改；它在多个测试文件中重复相同的 mock **样板代码**（Boilerplate——重复性高、几乎不需要思考的模板化代码）；它可能因为**模块缓存**（Node.js 会缓存已加载的模块，多次 `require` 同一模块返回同一个对象）导致测试之间的状态泄漏。
...
1. **天然的状态隔离**：每次调用对话函数都创建一个全新的**闭包**（Closure——函数与其周围变量的"打包"。闭包内的变量独立于外部，每次调用函数都会创建一套新的变量副本），所有可变状态都是函数局部变量。没有跨调用的状态泄漏风险。对比 class 方案：如果对话循环是一个 class 的方法，那么多个并发的对话实例可能意外共享 class 实例上的属性，导致状态污染。

2. **生成器的背压语义**：`yield` 暂停执行直到消费者请求下一个值。这意味着如果 UI 层无法跟上产出速度，生成器会自动暂停，而不会堆积内存。这是生产级系统必须考虑的问题——当工具执行产生大量输出（如 `npm install` 的完整日志）时，没有**背压**（Backpressure——当生产者速度超过消费者速度时，通过暂停生产者来防止数据堆积的机制）控制的系统可能因为内存堆积而崩溃。
...
对话循环的核心**状态机**（State Machine——一种用有限个"状态"和"转换规则"来描述系统行为的模型，系统在任意时刻只能处于一个状态，根据事件决定转换到哪个状态）由两个概念驱动：可变循环状态（State）和终止信号（Terminal）。
...
以下代码实现了第 2 章和第 8 章讨论的断路器模式。复制到 `circuit-breaker.ts` 后运行。

```typescript
// circuit-breaker.ts — 断路器模式（~40 行）
...
class CircuitBreaker {
  ...
  async execute<T>(fn: () => Promise<T>): Promise<T> {
    if (this.state === "open") {
      throw new Error("Circuit breaker is OPEN — skipping attempt");
    }
...
async function main() {
  ...
  for (let i = 0; i < 6; i++) {
    try {
      ...
      console.log(`  Attempt ${i + 1}: ${err.message} (state: ${breaker.getState()})`);
    }
  }
  ...
  try {
    const result = await breaker.execute(fakeCompact);
    console.log(`  Retry: ${result} (state: ${breaker.getState()})`);
  } catch (err: any) {
    console.log(`  Retry: ${err.message} (state: ${breaker.getState()})`);
  }
}
