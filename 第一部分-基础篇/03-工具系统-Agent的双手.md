...
**Rust 实现**（对应 `code/src/main.rs` 中的 `EditFileTool`）：

```rust
// 对应 Claude Code src/tools/EditFileTool 的 search-and-replace 逻辑
// Rust 使用 std::fs 读写文件，与 TS 的 fs.readFileSync 对应
...
fn edit_file(path: &str, old_string: &str, new_string: &str) -> Result<String, String> {
    ...
    // 抗幻觉检查：old_string 必须在文件中存在
    if !content.contains(old_string) {
        return Err(format!("Error: old_string not found in {path}"));
    }

    // 唯一性检查：old_string 必须只出现一次
    let count = content.matches(old_string).count();
    if count > 1 {
        return Err(format!("Error: old_string found {count} times in {path}. Must be unique."));
    }
...
> - TS 的 `content.includes(oldStr)` 对应 Rust 的 `content.contains(old_string)`
> - TS 的 `content.replace(oldStr, newStr)` 对应 Rust 的 `content.replace(old_string, new_string)`
> - TS 的 `throw new Error(...)` 对应 Rust 的 `return Err(format!(...))`
> - Rust 的 `Result<T, E>` 是编译时强制的错误处理，TS 的 `try/catch` 是运行时的

### 练习 4：运行 Rust 实现并对照源码

> **配套代码：** `code/src/main.rs` 用 Rust 实现了本章的核心概念。

```bash
cargo run -p ch03-tool-system    # 运行演示
cargo test -p ch03-tool-system   # 运行 10 个测试
```
...
|-----------|-----------------|---------|
| `ToolHandler` trait | `src/Tool.ts` Tool 接口 | 3.1 工具定义协议 |
| `ToolRegistry` | `src/tools.ts` getAllBaseTools() | 3.2 工具注册 |
| `ToolRouter` | `src/services/tools/toolOrchestration.ts` | 3.4 工具编排引擎 |
| `CONCURRENCY_SAFE_TOOLS` | isConcurrencySafe() | 3.4 并发分区 |

**Rust 特有洞察：** 为什么 `ToolHandler` trait 要求 `Send + Sync`？（提示：Rust 的 `Send` 表示可跨线程传递，`Sync` 表示可跨线程共享引用——这在编译期强制保证了工具的并发安全属性，对应 Claude Code 中 `isConcurrencySafe()` 的运行时检查）

**修改实验：** 在 `code/src/main.rs` 中将 `GrepTool` 的并发安全改为 `false`，运行 `cargo run -p ch03-tool-system` 观察分区结果变化。
