...
- 掌握 Claude Code 66+ 工具的设计模式，理解五要素协议的设计哲学
- 理解工具定义协议、注册机制、**编排引擎**（Orchestration Engine——协调多个组件按特定顺序和策略协同工作的调度中心）的完整架构
- 分析**并发分区**（Concurrency Partitioning——将任务分为可并行和必须串行的组，分别执行）策略的调度原理和实际效果
- 理解 StreamingToolExecutor 四阶段**状态机**（State Machine——用有限个状态和转换规则描述系统行为的模型）的精妙设计
- 评估延迟工具发现机制的工程价值

---

Maslow 的这句话用在 Agent 工具系统上再贴切不过。如果 Agent 只有一个 Bash 工具，所有任务都会变成 Shell 命令——读取文件用 `cat`，搜索代码用 `grep`，编辑文件用 `sed`。这虽然可行，但违反了"使用正确工具解决正确问题"的工程原则。Claude Code 的工具系统提供了 66+ 个专门化工具，每个工具针对特定的操作类型做了优化——这就好比为不同任务配备不同的专业工具，而不是用一把锤子解决所有问题。
...
`buildTool` 是创建工具的标准工厂函数。它接收一个部分工具定义，自动填充安全默认值。这些默认值遵循"fail-closed"（安全设计原则：遇到未知情况时默认采取最严格的限制措施，如拒绝执行）原则：安全性相关的方法（如并发安全判断、只读判断）默认为 false，工具必须显式声明自己安全才能享受并发等优化。
...
Claude Code 的工具注册大量使用条件导入实现编译期**死代码消除**（Dead Code Elimination——编译器或打包工具移除程序中永远不会被执行到的代码，减小最终产物的体积）。当特定条件不满足时，对应工具的整个模块不会被包含在最终构建中。功能开关控制的工具也是如此。

功能开关来自构建工具链，在编译时由 bundler 评估。当功能开关关闭时，对应的工具实现代码都会被 **tree-shaking**（JavaScript 打包工具如 Webpack、esbuild 的优化技术，通过静态分析移除未被导入的模块代码）移除。这种模式确保了外部构建（面向第三方用户）不会包含内部工具的代码。
...
这三个工具构成了 Claude Code 文件操作的完整能力集。它们的分工反映了经典数据库操作的 **CRUD** 模式（Create/Read/Update/Delete——创建、读取、更新、删除，软件系统中最基本的四种数据操作），只是缺少了 Delete——这是一个有意为之的安全决策，因为"删除文件"是不可逆操作，通常通过 BashTool 的 `rm` 命令来实现，这会触发更严格的权限检查。
...
- **为什么不用行号？** 行号是脆弱的——如果在读取文件和编辑文件之间，另一个工具（或用户）修改了文件，行号可能已经偏移，导致编辑错误的位置。更关键的是，当模型在同一个 turn 中对同一文件做多处修改时，第一个编辑会导致后续所有行号偏移。
- **为什么不用 AST？** AST 编辑在理论上优雅，但实际不可行——Claude Code 需要支持几十种语言，为每种维护 AST 解析器成本极高。更致命的是：**语法错误的文件恰恰是最需要编辑的文件**，但 AST 解析器遇到语法错误时会直接拒绝解析。
- **为什么不用 unified diff？** LLM 在生成严格格式（精确的 hunk header、`+`/`-`/空格前缀）时表现很差。任何一个字符的偏差都会导致整个 patch 无法应用。
- **为什么用精确字符串匹配？** 字符串匹配是**幂等的**（Idempotent——执行一次和执行多次的效果完全相同）——只要目标字符串存在于文件中，编辑就能正确定位。即使文件被部分修改，只要目标片段没有被触及，编辑就是安全的。

**抗幻觉特性：** FileEditTool 最被低估的优势是它的抗幻觉能力。考虑这个场景：模型"记得"文件中有一个 `handleError()` 函数，但实际上它在上一次重构中已被重命名为 `processError()`。使用 search-and-replace，模型提供 `old_string: "function handleError()"` 会直接失败（"String to replace not found in file"），模型看到错误后会重新读取文件，发现正确的函数名。如果使用全文件重写，模型可能会写出包含 `handleError()` 的完整文件，覆盖掉正确的 `processError()`——而且这个错误完全是静默的，不会有任何报错。

**输入预处理：** 模型的编辑输入在验证前会经过预处理：(1) 尾部空白裁剪（`.md` 文件例外，因为 Markdown 行尾双空格表示硬换行）；(2) API 反消毒——Claude API 会将某些 XML 标签消毒为短形式（如 `<function_results>` → `<fnr>`），预处理会尝试还原这些标签以确保匹配成功。
...
**GrepTool** 使用**正则表达式**（Regular Expression，简称 regex——一种用特殊语法描述文本模式的工具，如 `\d+` 匹配一个或多个数字）搜索文件内容，底层使用 `ripgrep`。它支持多种输出模式（文件名、内容行、计数），以及丰富的过滤选项（文件类型、glob 模式等）。
...
工具编排引擎是工具系统的"指挥中心"——它决定了多个工具调用如何被调度、执行和结果收集。一个好的编排引擎需要在三个目标之间取得平衡：**并行性**（尽可能并行执行以提高速度）、**安全性**（避免并发执行导致的**数据竞争**——多个操作同时读写同一数据，导致结果取决于执行顺序的 bug）和**顺序性**（保证结果的产出顺序与请求顺序一致）。
...
5. **信号传播**：每个工具执行使用独立的子取消控制器（**AbortController**——Web API 提供的中止控制器，用于取消异步操作，通过 signal 信号通知操作应被终止），形成层级化的取消信号链。兄弟工具的错误或用户的 Ctrl+C 都会通过信号传播到正确的工具。这种层级化的信号传播确保了取消操作的精确性——取消一个工具不会意外影响不相关的其他工具。
...
以下代码实现了第 3 章的核心概念——Tool 五要素协议、buildTool 工厂函数、并发分区算法。复制到 `mini-tools.ts` 后用 `npx tsx mini-tools.ts` 运行。

```typescript
// mini-tools.ts — 最小工具系统（~120 行）
// 核心概念：Tool 接口 + buildTool 工厂 + 并发分区 + 流式执行状态机
...
interface Tool<I extends Record<string, unknown>, O> {
  name: string;                           // 要素一：名称
  inputSchema: z.ZodType<I>;              // 要素二：Zod Schema
  isReadOnly: (input: I) => boolean;      // 要素三：权限模型
  isConcurrencySafe: (input: I) => boolean;
  call: (input: I) => Promise<O>;         // 要素四：执行逻辑
  render: (input: I, output: O) => string; // 要素五：UI 渲染
}

// ── buildTool 工厂函数（对应 3.1 节 buildTool） ────────
function buildTool<I extends Record<string, unknown>, O>(
  def: Partial<Tool<I, O>> & Pick<Tool<I, O>, "name" | "inputSchema" | "call">,
): Tool<I, O> {
  return {
    ...def,
    isReadOnly: def.isReadOnly ?? (() => false),        // fail-closed 默认
...
const BashTool = buildTool({
  name: "bash",
  inputSchema: z.object({ command: z.string().describe("Shell 命令") }),
  isReadOnly: () => false,
  isConcurrencySafe: () => false,  // 有副作用 → 不可并发
  call: async (input) => {
...
const GrepTool = buildTool({
  name: "grep",
  inputSchema: z.object({ pattern: z.string().describe("搜索模式") }),
  isReadOnly: () => true,
...
// ── 并发分区算法（对应 3.4 节 partitionToolCalls） ─────
...
function partitionToolCalls(calls: ToolCall[]): ToolCall[][] {
  ...
  for (const call of calls) {
    ...
    if (currentBatch.length === 0) {
      ...
      currentBatch.push(call);  // 同为安全 → 合并到同一批次
    } else {
...
// ── 模拟执行 ────────────────────────────────────────────
async function executeBatch(batch: ToolCall[]): Promise<Map<string, string>> {
  const results = new Map<string, string>();
  if (batch.length > 1 && batch.every(c => c.tool.isConcurrencySafe(c.input))) {
    // 并行执行
    const promises = batch.map(async (c) => {
      const output = await c.tool.call(c.input);
      results.set(c.id, c.tool.render(c.input, output));
    });
...
async function main() {
  ...
  [ReadTool, BashTool, GrepTool].forEach(t => toolRegistry.set(t.name, t));
  ...
  batches.forEach((batch, i) => {
    const names = batch.map(c => `${c.tool.name}(${JSON.stringify(c.input)})`);
    const safe = batch.every(c => c.tool.isConcurrencySafe(c.input));
    console.log(`  Batch ${i + 1} [${safe ? "并行" : "串行"}]: ${names.join(", ")}`);
  });
  ...
  for (let i = 0; i < batches.length; i++) {
    console.log(`\nBatch ${i + 1}:`);
    const results = await executeBatch(batches[i]);
    results.forEach((render, id) => console.log(`  [${id}] ${render}`));
  }
}
...
**运行后观察：**
- 并发分区如何将 `[Grep, Read, Bash, Read, Grep]` 划分为 3 个批次
- 只读工具（Grep、Read）被合并到同一个并行批次
- 有副作用的 Bash 工具被单独隔离为串行批次
- buildTool 的 fail-closed 默认值如何工作

**修改实验：**
1. 将 GrepTool 的 `isConcurrencySafe` 改为 `false`，观察分区结果变化
2. 添加一个 `WriteTool`（`isReadOnly: () => false`），观察它如何被隔离
3. 给 BashTool 添加 `isDestructive: () => true`，思考这对权限系统的影响

### 练习 2：手推并发分区算法

给定以下工具调用序列，手动模拟 `partitionToolCalls` 的决策过程：

```
[Glob(*.ts), Grep(pattern), Bash(npm test), Read(a.ts), Edit(a.ts), Glob(*.json)]
```

逐步记录每一步的 `currentSafe` 和 `currentBatch` 状态。正确答案应该是：

| Batch | 工具 | 并发安全 | 执行方式 |
|-------|------|---------|---------|
| 1 | Glob, Grep | 都是 true | 并行 |
| 2 | Bash | false | 串行 |
| 3 | Read | true | 串行（前一个 batch 是 unsafe） |
| 4 | Edit | false | 串行 |
| 5 | Glob | true | 串行（前一个 batch 是 unsafe） |

**关键洞察：** Read(a.ts) 虽然自身是并发安全的，但因为前一个批次是 unsafe 的 Bash，它不能回溯合并到 Batch 1。这是"顺序保证"的体现——结果的产出顺序必须与请求顺序一致。

### 练习 3：对比 search-and-replace vs 全文件重写

以下代码演示了 FileEditTool 的抗幻觉特性（对应 3.3 节）：

```typescript
// anti-hallucination-demo.ts
...
const correctOldString = "function processError(err: Error)";

// search-and-replace：幻觉会导致明确的错误
function searchAndReplace(content: string, oldStr: string, newStr: string): string {
  if (!content.includes(oldStr)) {
    throw new Error(`String to replace not found: "${oldStr.slice(0, 40)}..."`);
  }
  return content.replace(oldStr, newStr);
}

// 全文件重写：幻觉会导致静默的错误
function fullRewrite(content: string, newContent: string): string {
  return newContent;  // 直接覆盖，不检查旧内容
}

// 测试 search-and-replace
console.log("=== search-and-replace ===");
try {
  searchAndReplace(fileContent, hallucinatedOldString, "function fixed() {}");
} catch (err: any) {
  console.log(`  ✅ 幻觉被检测到: ${err.message}`);
}

// 测试全文件重写
console.log("\n=== full file rewrite ===");
const badRewrite = fullRewrite(fileContent, `
function handleError(err: Error) {  // 幻觉的旧名
  console.log(err.message);
}
`);
console.log(`  ❌ 幻觉未被检测到，文件被静默覆盖`);
console.log(`  原文有 processError，重写后有 handleError — 数据丢失！`);
```

**核心结论：** search-and-replace 的 `old_string` 必须精确匹配文件内容，否则直接报错——这是"抗幻觉"的关键机制。全文件重写无法检测这类错误。
