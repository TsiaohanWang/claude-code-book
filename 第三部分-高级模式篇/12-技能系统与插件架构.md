...
核心内置技能的完整清单如下（基于官方文档 code.claude.com/docs/en/skills）：

| 技能名称 | 用途 | 用户可调用 | 典型使用场景 |
|---------|------|-----------|------------|
| `/code-review` | 代码审查 | 是 | PR 审查、代码质量检查 |
| `/batch` | 批量文件处理 | 是 | 批量重命名、批量格式化 |
| `/debug` | 调试辅助 | 是 | 定位 bug 根因、分析错误堆栈 |
| `/loop` | 定时循环执行 | 是 | 轮询任务、定时检查 |
| `/claude-api` | Claude API 开发辅助 | 是 | 构建基于 Claude API 的应用 |
| `/run` | 启动并驱动应用 | 是 | 查看变更效果 |
| `/verify` | 构建并验证变更 | 是 | 确认代码变更正确性 |
| `/run-skill-generator` | 录制启动配方 | 是 | 为 /run 和 /verify 生成项目专属技能 |
...
文件提取采用了精巧的惰性单例模式，使用 `??=` 操作符确保并发调用共享同一个提取 Promise，避免竞态条件。文件写入使用 `O_NOFOLLOW | O_EXCL` 标志防止**符号链接**（Symbolic Link——文件系统中指向另一个文件的特殊文件，类似快捷方式）攻击，目录权限设置为 `0o700`，文件权限设置为 `0o600`。
...
> **交叉引用：** 插件提供的智能体（agents/）使用与第 10 章中自定义智能体相同的 Markdown 格式。区别在于来源——自定义智能体来自 `.claude/agents/` 目录，插件智能体来自插件的 `agents/` 目录，两者的优先级不同。
...
### 12.6.1 动态上下文注入（`!`command`` 语法）

技能支持在 SKILL.md 中嵌入 Shell 命令，命令输出会在技能加载时替换到内容中。这是 Claude Code 技能系统最强大的特性之一。
...
`!`git diff HEAD`` 在技能加载时执行，输出替换到原位。Claude 看到的是实际的 diff 内容，而非命令本身。
...
> **安全边界：** MCP 技能来自远端服务器，不执行 Shell 命令内联展开。本地技能文件由用户创建（可信），MCP 技能来自网络（不可信）。

### 12.6.2 技能内容生命周期

技能内容在调用后进入对话上下文，**在整个会话期间持续存在**。Claude Code 不会在后续轮次重新读取技能文件。

自动压缩（Auto-compaction）时，技能内容会被保留：每个技能保留前 5,000 tokens，总预算 25,000 tokens。最近调用的技能优先保留。

> **设计启示：** 技能内容越精简越好。每行都是持续的 token 成本。用 CLAUDE.md 存放常驻事实，用技能存放按需加载的流程。

### 12.6.3 调用控制

| frontmatter 字段 | 用户可调用 | Claude 可调用 | 适用场景 |
|------------------|-----------|--------------|---------|
| 默认 | ✅ | ✅ | 通用技能 |
| `disable-model-invocation: true` | ✅ | ❌ | 有副作用的操作（部署、发布） |
| `user-invocable: false` | ❌ | ✅ | 后台知识（领域知识、上下文） |

```yaml
---
name: deploy
description: Deploy to production
disable-model-invocation: true   # 只有用户能触发
...
### 12.6.4 Agent Skills 开放标准

Claude Code 的技能系统遵循 [Agent Skills](https://agentskills.io) 开放标准，该标准定义了：

- **SKILL.md 格式**：YAML frontmatter + Markdown 内容
- **目录结构**：`skills/<name>/SKILL.md` 入口文件
- **参数替换**：`$ARGUMENTS`、`$0`、`$1` 等占位符
- **动态上下文**：`!`command`` 语法

这意味着同一个 SKILL.md 文件可以在 Claude Code、Cursor、Windsurf 等支持 Agent Skills 标准的工具中使用。

> **Codex 对比：** Codex 使用 `codex-rs/skills/` 和 `codex-rs/core-skills/` 管理技能。Codex 的技能通过 `install_system_skills()` 安装到 `CODEX_HOME/skills/.system/`，使用 fingerprint marker 避免重复安装。Claude Code 的技能更轻量（纯 Markdown），Codex 的技能更结构化（Rust crate + asset embedding）。
...
```rust
// 对应 Claude Code src/skills/ 的技能系统
// SKILL.md 解析 + 前端matter + 技能选择
...
// 技能选择：根据用户输入匹配最合适的技能
fn select_skill(query: &str, skills: &[SkillDef]) -> Option<&SkillDef> {
    let q = query.to_lowercase();
    skills.iter().find(|s| {
        q.contains(&s.name) || q.contains(s.trigger.split_whitespace().last().unwrap_or(""))
    })
}
...
---
") {
        let frontmatter = &content[4..end]; // skip "---
"
...
> **Rust vs TS 差异：** Rust 的 `Option<&SkillDef>` 用类型系统强制调用者处理"未找到"的情况。TS 返回 `SkillDef | null`，调用者可能忘记检查 null。

## 实战练习

### 练习 1：运行技能系统

以下代码实现了第 12 章的核心概念——技能发现、选择、Fork/inline 上下文。复制到 `mini-skills.ts` 后用 `npx tsx mini-skills.ts` 运行。

> **源码参考：** 对应 Claude Code `src/skills/SkillTool.ts` 中的技能执行和 `src/skills/` 目录中的内置技能定义。

```typescript
// mini-skills.ts — 最小技能系统（~40 行）
// 源码参考：Claude Code src/skills/

interface SkillDef { name: string; description: string; trigger: string; tools: string[]; context: "inline" | "fork"; }

const BUILTIN_SKILLS: SkillDef[] = [
  { name: "search", description: "Search codebase", trigger: "find code", tools: ["grep_search", "list_files"], context: "inline" },
  { name: "explain", description: "Explain code", trigger: "explain how", tools: ["read_file", "grep_search"], context: "inline" },
  { name: "test", description: "Write/run tests", trigger: "write tests", tools: ["read_file", "write_file", "run_shell"], context: "inline" },
];

function selectSkill(query: string, skills: SkillDef[]): SkillDef | null {
  const q = query.toLowerCase();
  return skills.find(s => q.includes(s.name) || q.includes(s.trigger.split(" ").pop()!)) || null;
}

function main() {
  console.log("=== 技能系统测试 ===\n");
  console.log("1. 内置技能:");
  BUILTIN_SKILLS.forEach(s => console.log(`  ${s.name}: ${s.description} (${s.context})`));
  console.log("\n2. 技能选择:");
  ["find all TODO comments", "explain how auth works", "write tests for utils"].forEach(q => {
    const s = selectSkill(q, BUILTIN_SKILLS);
    console.log(`  "${q}" → ${s?.name || "no match"}`);
  });
  console.log("\n3. Fork vs inline 上下文:");
  console.log(`  inline: 在主上下文中运行，共享消息历史`);
  console.log(`  fork: 在子智能体中运行，隔离上下文，不污染主对话`);
}
main();
```
