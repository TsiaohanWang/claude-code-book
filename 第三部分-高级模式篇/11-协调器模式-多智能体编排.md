...
> **交叉引用：** Fork 模式的缓存共享机制在第 10 章中详细讨论。两者的核心区别在于：Fork 是"无中心的并行"（所有子智能体平等，共享相同上下文），而 Coordinator 是"有中心的编排"（协调者控制全局，工作者只看到分配给自己的任务）。

Coordinator 模式还影响智能体注册表。当 Coordinator 模式激活时，内置智能体注册函数不再返回普通的内置智能体列表，而是通过**懒加载**（Lazy Loading——延迟初始化策略，只在首次需要时才加载资源，减少启动时间和内存占用）引入 Coordinator 专用的工作者智能体定义。这种懒加载方式是有意为之的，目的是避免协调器模块与工具模块之间的**循环依赖**（模块 A 依赖模块 B，模块 B 又依赖模块 A，导致加载时出现初始化顺序问题）。
...
## 11.7 Coordinator 模式与 Fork 模式对比
...
> **交叉引用：** Fork 模式的详细机制参见第 10 章。特别注意两者的互斥关系——Coordinator 模式激活时 Fork 模式被自动禁用。
...
```rust
// 对应 Claude Code 的 Coordinator-Worker 模式
// 核心约束：协调器只编排，不执行工具

enum AgentType { Explore, Plan, General }

struct Task { id: String, agent_type: AgentType, description: String }
struct TaskResult { task_id: String, output: String, success: bool }

struct Coordinator { agents: Vec<AgentType> }

impl Coordinator {
    // 核心约束：协调器不能执行工具
    fn can_execute_tools(&self) -> bool { false }

    // 目标分解 → 任务分配 → 串行执行
    async fn run(&self, goal: &str) -> Vec<TaskResult> {
        let tasks = self.decompose(goal);
    ...
    fn decompose(&self, goal: &str) -> Vec<Task> {
        vec![
            Task { id: "t1".into(), agent_type: AgentType::Explore, description: format!("Find files for: {goal}") },
            Task { id: "t2".into(), agent_type: AgentType::Plan, description: format!("Design plan for: {goal}") },
            Task { id: "t3".into(), agent_type: AgentType::General, description: format!("Implement: {goal}") },
        ]
    }

    async fn execute_task(&self, task: &Task) -> TaskResult {
        // 每个任务分配给对应类型的 Agent
        TaskResult { task_id: task.id.clone(), output: "done".into(), success: true }
    }
}
```

> **Rust vs TS 差异：** Rust 的 `async fn` 返回 `impl Future`，需要 tokio 运行时。Rust 的 `Vec<Task>` 拥有所有权，传递给函数后原变量不可再用——这天然保证了任务不会被重复执行。

## 实战练习

### 练习 1：运行协调器模式

以下代码实现了第 11 章的核心概念——Coordinator-Worker 双重门控、"只编排不执行"约束、任务分解与串行执行。复制到 `mini-coordinator.ts` 后用 `npx tsx mini-coordinator.ts` 运行。

> **源码参考：** 对应 Claude Code `src/agents/` 目录中的协调器模式实现和 `src/tools/AgentTool.ts` 中的子智能体调度。

```typescript
// mini-coordinator.ts — 最小协调器（~60 行）
// 源码参考：Claude Code src/agents/, src/tools/AgentTool.ts

type AgentType = "explore" | "plan" | "general";
interface AgentTask { id: string; type: AgentType; description: string; status: "pending" | "running" | "done" | "failed"; result?: string; }
interface Agent { id: string; type: AgentType; busy: boolean; }

class Coordinator {
  private agents: Agent[] = [
    { id: "explore-1", type: "explore", busy: false },
    { id: "plan-1", type: "plan", busy: false },
    { id: "general-1", type: "general", busy: false },
  ];

  // Rule: coordinator only orchestrates, never executes
  canExecuteTools(): boolean { return false; }

  decomposeGoal(goal: string): AgentTask[] {
    console.log(`  📋 分解目标: "${goal}"`);
    return [
      { id: "t1", type: "explore", description: `查找相关文件: ${goal}`, status: "pending" },
      { id: "t2", type: "plan", description: `设计方案: ${goal}`, status: "pending" },
      { id: "t3", type: "general", description: `实现: ${goal}`, status: "pending" },
    ];
  }

  async executeTask(task: AgentTask): Promise<void> {
    const agent = this.agents.find(a => a.type === task.type && !a.busy);
    if (!agent) { task.status = "failed"; return; }
    agent.busy = true; task.status = "running";
    console.log(`    🔀 ${task.id} → ${agent.id}`);
    await new Promise(r => setTimeout(r, 50));
    task.status = "done"; task.result = `[${agent.id}] 完成`; agent.busy = false;
    console.log(`    ✅ ${task.id} done`);
  }

  async run(goal: string): Promise<void> {
    if (this.canExecuteTools()) throw new Error("Coordinator must not execute tools!");
    const tasks = this.decomposeGoal(goal);
    for (const task of tasks) await this.executeTask(task);
    console.log(`  📊 ${tasks.filter(t => t.status === "done").length}/${tasks.length} 完成`);
  }
}

async function main() {
  console.log("=== 协调器模式测试 ===\n");
  console.log("1. 只编排不执行:");
  const coord = new Coordinator();
  console.log(`  canExecuteTools(): ${coord.canExecuteTools()}\n`);
  console.log("2. 目标分解与执行:");
  await coord.run("添加用户认证模块");
}
main();
```
...
基于本章和第 10 章的内容，填写下表并给出每个维度的选择理由：
