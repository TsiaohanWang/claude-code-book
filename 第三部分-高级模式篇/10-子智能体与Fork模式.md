...
> **交叉引用：** 插件智能体的加载机制与第 12 章的插件系统紧密相关。插件不仅可以提供技能（Skill），还可以提供智能体（Agent），两者共享相同的插件发现和加载基础设施。
...
| 资源节省 | **Copy-on-Write**（写时复制——fork 后父子进程共享物理内存，只有在某一方写入时才复制该页，节省内存）内存共享 | Prompt Cache token 共享 |
...
> **交叉引用：** Coordinator 模式与 Fork 模式的互斥关系将在第 11 章详细讨论。简言之，Coordinator 是一种更重量的编排模式，适合需要精细任务管理的场景；Fork 是一种轻量级的并行模式，适合将同一上下文分发到多个独立子任务。
...
> **交叉引用：** 自定义智能体的 hooks 配置与第 9 章的 Hook 系统使用相同的格式。你可以为智能体定义 PreToolUse、PostToolUse 等生命周期钩子，实现诸如审计日志、自动格式化等横切关注点。
...
> **设计考量：为什么 MCP 工具始终可用？**
>
> MCP 工具是由用户显式配置的外部服务，它们经过了用户授权（通过权限配置）。与内置工具不同，MCP 工具的行为由外部服务器定义，Claude Code 无法预知其功能范围。因此，系统选择不限制子智能体使用 MCP 工具，而是将安全控制交给 MCP 的权限系统（参见第 13 章）。
...
以下代码实现了第 10 章的核心概念——三种 Agent 来源、工具隔离、Fork 上下文继承。复制到 `mini-subagent.ts` 后用 `npx tsx mini-subagent.ts` 运行。

> **源码参考：** 对应 Claude Code `src/tools/AgentTool.ts` 中的子智能体创建和 `src/agents/` 目录中的内置 Agent 定义。

```typescript
// mini-subagent.ts — 最小子智能体系统（~50 行）
// 源码参考：Claude Code src/tools/AgentTool.ts, src/agents/

type SubAgentType = string;
interface SubAgentConfig { systemPrompt: string; tools: string[]; }

const AGENT_PROMPTS: Record<string, string> = {
  explore: "READ-ONLY mode. Use list_files and grep_search. Parallel tool calls.",
  plan: "READ-ONLY. Analyze architecture, design step-by-step plans.",
  general: "Complete the task fully. Search broadly, then narrow down.",
};
const READ_ONLY = ["read_file", "list_files", "grep_search"];

function getSubAgentConfig(type: SubAgentType): SubAgentConfig {
  if (AGENT_PROMPTS[type]) {
    return { systemPrompt: AGENT_PROMPTS[type], tools: type === "general" ? [...READ_ONLY, "write_file", "edit_file", "run_shell"] : READ_ONLY };
  }
  return { systemPrompt: AGENT_PROMPTS.general, tools: [] };
}

async function main() {
  console.log("=== 子智能体 Fork 模式测试 ===\n");

  for (const type of ["explore", "plan", "general"]) {
    const config = getSubAgentConfig(type);
    console.log(`${type}: ${config.tools.length} tools [${config.tools.join(", ")}]`);
    console.log(`  只读: ${config.tools.every(t => READ_ONLY.includes(t))}`);
  }

  console.log("\n工具隔离验证:");
  const explore = getSubAgentConfig("explore");
  const general = getSubAgentConfig("general");
  console.log(`  Explore 有 write_file: ${explore.tools.includes("write_file")}`);
  console.log(`  General 有 write_file: ${general.tools.includes("write_file")}`);
}
main();
```
...
```bash
cargo run -p ch10-multi-agent    # 运行演示
cargo test -p ch10-multi-agent   # 运行测试
```
...
**修改实验：** 尝试修改 `code/src/main.rs` 中的关键参数，运行 `cargo run -p ch10-multi-agent` 观察行为变化。
