...
> **交叉引用：** Teammate 的 mailbox 机制在第 11 章"协调器模式 -- 多智能体编排"中有详细分析。Plan 模式的 teammate 路径是分布式 Agent 协作的一个典型案例 -- 审批决策的权力被委托给 team lead，而非本地用户。
...
> **交叉引用：** Fork Agent 的创建和状态隔离机制在第 10 章"子智能体与 Fork 模式"中有详细分析。计划文件的 fork 策略是 Agent 状态隔离的一个具体体现。
...
> **交叉引用：** Skill 系统的完整架构在第 12 章"技能系统与插件架构"中有详细分析。Hook 系统在第 9 章"钩子系统 -- Agent 的生命周期扩展点"中讨论。MCP 集成在第 13 章"MCP 集成与外部协议"中介绍。
