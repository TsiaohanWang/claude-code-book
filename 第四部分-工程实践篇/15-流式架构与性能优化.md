...
## 15.2 StreamingToolExecutor 的并发控制
...
> **交叉引用：** 上下文压缩策略直接影响缓存效率。第 7 章"上下文管理 -- Agent 的工作记忆"详细分析了五级压缩策略，每次压缩都会重写消息历史，可能导致缓存断裂。设计压缩策略时需要在"释放 token 空间"和"保持缓存连续性"之间取得平衡。
...
以下代码实现了第 15 章的核心概念——AsyncGenerator 流式输出、StreamingToolExecutor 四阶段状态机、并发安全分区。复制到 `mini-streaming.ts` 后用 `npx tsx mini-streaming.ts` 运行。

> **源码参考：** 对应 Claude Code `src/services/tools/StreamingToolExecutor.ts` 中的流式执行器和 `src/services/tools/toolOrchestration.ts` 中的并发分区。

```typescript
// mini-streaming.ts — 最小流式架构（~80 行）
// 源码参考：Claude Code src/services/tools/StreamingToolExecutor.ts

// ── AsyncGenerator 流式输出 ──────────────────────────────
async function* streamResponse(tokens: string[]): AsyncGenerator<{ type: string; data: string }, void, void> {
  for (const token of tokens) { await new Promise(r => setTimeout(r, 30)); yield { type: "delta", data: token }; }
  yield { type: "stop", data: "" };
}

// ── StreamingToolExecutor 四阶段状态机 ───────────────────
type ToolState = "queued" | "executing" | "completed" | "yielded";
interface TrackedTool { id: string; name: string; state: ToolState; result?: string; }
...
class StreamingToolExecutor {
  ...
  enqueue(name: string): string {
    const id = `t${this.queue.length + this.executing.length + 1}`;
    this.queue.push({ id, name, state: "queued" }); return id;
  }

  canStart(tool: TrackedTool, safe: Set<string>): boolean {
    if (!this.executing.length) return true;
    return this.executing.every(t => safe.has(t.name)) && safe.has(tool.name);
  }

  async processNext(safe: Set<string>, exec: (name: string) => Promise<string>): Promise<TrackedTool | null> {
    const next = this.queue.find(t => this.canStart(t, safe));
    if (!next) return null;
    this.queue = this.queue.filter(t => t !== next);
    next.state = "executing"; this.executing.push(next);
    next.result = await exec(next.name);
    next.state = "completed"; this.executing = this.executing.filter(t => t !== next);
    next.state = "yielded"; return next;
  }
}
...
async function main() {
  ...
  console.log("1. 流式 token 输出:");
  process.stdout.write("  ");
  for await (const e of streamResponse(["Hello", ",", " world", "!"])) {
    if (e.type === "delta") process.stdout.write(e.data);
  }
  console.log("\n");

  console.log("2. StreamingToolExecutor:");
  const executor = new StreamingToolExecutor();
  const safe = new Set(["read_file", "grep_search"]);
  ["read_file", "grep_search", "bash", "read_file"].forEach(n => executor.enqueue(n));
  console.log("  队列: read_file, grep_search, bash, read_file");
  let tool;
  while ((tool = await executor.processNext(safe, async n => `result(${n})`)))
    console.log(`  ${tool.id} [${tool.name}]: ${tool.state} — ${tool.result}`);

  console.log("\n3. 并发安全分区:");
  console.log("  Batch 1 [并行]: read_file + grep_search (都是安全工具)");
  console.log("  Batch 2 [串行]: bash (非安全)");
  console.log("  Batch 3 [串行]: read_file (前一个 batch 是 unsafe)");
}
main();
```
...
**Rust 实现**（对应 `code/src/main.rs` 中的流式处理）：

```rust
// 对应 Claude Code src/services/tools/StreamingToolExecutor.ts

// SSE 事件解析（对应 src/services/api/streaming.ts）
enum StreamEvent {
    TextDelta(String),
...
// 并发安全工具集（对应 isConcurrencySafe()）
const CONCURRENCY_SAFE_TOOLS: &[&str] = &["read_file", "list_files", "grep_search", "web_fetch"];

// 流式工具执行器 —— 四阶段状态机
enum ToolState { Queued, Executing, Completed, Yielded }

struct StreamingToolExecutor {
    queue: Vec<TrackedTool>,
    executing: Vec<TrackedTool>,
}

impl StreamingToolExecutor {
    fn can_start(&self, tool: &TrackedTool) -> bool {
        if self.executing.is_empty() { return true; }
        // 只有当所有执行中的工具都是并发安全时，才能启动新工具
        self.executing.iter().all(|t| CONCURRENCY_SAFE_TOOLS.contains(&t.name.as_str()))
            && CONCURRENCY_SAFE_TOOLS.contains(&tool.name.as_str())
    }

    async fn process_next(&mut self) -> Option<TrackedTool> {
        let next = self.queue.iter().find(|t| self.can_start(t)).cloned()?;
        self.queue.retain(|t| t.id != next.id);
        // ... execute and return
        Some(next)
    }
}
...
> **Rust vs TypeScript 差异：** Rust 的 `Vec::retain` 在原地过滤元素，比 TS 的 `filter` 创建新数组更高效。Rust 的所有权系统确保 `TrackedTool` 在移动到 `executing` 后不再被 `queue` 引用——编译器强制保证了状态一致性。
...
```bash
cargo run -p ch07-streaming    # 运行演示
cargo test -p ch07-streaming   # 运行测试
```
...
**修改实验：** 尝试修改 `code/src/main.rs` 中的关键参数，运行 `cargo run -p ch07-streaming` 观察行为变化。
