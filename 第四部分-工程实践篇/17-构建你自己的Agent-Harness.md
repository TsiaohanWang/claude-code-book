...
Schema 驱动的深层价值在于"**单一真相源**"（Single Source of Truth——架构原则，确保每个数据只有一个权威来源，避免多处维护导致不一致）。考虑一个没有 Schema 驱动的替代方案：工具的输入验证写在一处，权限检查写在另一处，API 文档又是手动编写的。当参数需要修改时，三处都需要同步更新，遗漏任何一处都会导致不一致。Schema 驱动确保了验证逻辑、权限逻辑和文档都是从同一个定义派生的，修改一处即全局生效。
...
- **错误重试**：模型调用失败时的重试逻辑，包括**指数退避**（Exponential Backoff——重试等待时间按指数增长如 1s→2s→4s）和**降级策略**（Fallback——主方案失败时自动切换到备用方案）。
