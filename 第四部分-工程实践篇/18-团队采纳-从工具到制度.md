...
**Rust 实现**（CLAUDE.md 加载器）：

```rust
// 对应 Claude Code src/prompt.ts:75-97 的 loadClaudeMd()

fn load_claude_md(cwd: &std::path::Path) -> String {
...
> **关键设计：** CLAUDE.md 从文件系统根目录向下加载，越靠近当前目录的越后读取（后读取的优先级更高）。
