# 第 9 章 钩子系统 -- Agent 的生命周期扩展点

> **学习目标：** 阅读本章后，你将能够：
>
> - 理解五种钩子类型的设计哲学、能力边界和适用场景
> - 掌握 31 个生命周期事件的触发时机、输入结构和退出码语义
...
从工程角度看，钩子系统遵循的是经典的**观察者模式**（Observer Pattern——当对象状态变化时自动通知所有注册的观察者）结合**责任链模式**（Chain of Responsibility——请求沿着处理者链传递，每个处理者决定是处理还是转发）。每个生命周期事件就是一个"信号"，多个钩子可以监听同一个信号，按优先级依次处理，任何一个钩子都可以选择"阻断信号传播"。这种设计使得 Claude Code 的核心执行引擎无需了解扩展逻辑的存在，实现了核心系统与用户自定义逻辑的彻底解耦。
...
钩子 Schema 定义模块中定义了五种钩子类型。下面这张对比表可以帮助你快速理解每种钩子的定位和能力边界：
...
graph TD
    subgraph 五种钩子类型["五种钩子类型能力图谱"]
        ...
        mcp["MCP Tool 钩子<br/>──────────<br/>执行引擎: MCP 服务器<br/>可持久化: 是<br/>配置格式: JSON<br/>复杂度: 中<br/>延迟: 网络依赖<br/>典型场景: 外部工具集成"]
    end
...
**5. MCP Tool 钩子**（`type: "mcp_tool"`）

调用已连接的 MCP 服务器上的工具作为钩子。工具的文本输出被当作 Command 钩子的 stdout 处理。这允许将外部 MCP 工具集成到钩子系统中，实现更复杂的工作流。

适用场景：调用外部代码质量检测服务、触发 CI/CD 流程、与第三方系统集成。

> **内部机制补充：** 除上述五种用户可配置的钩子类型外，Claude Code 内部还有一种 `FunctionHook`——仅在运行时存在的 TypeScript 回调函数，无法持久化到配置文件。它用于 SDK 嵌入模式下的动态行为控制，生命周期与会话绑定。这种"声明式配置 vs 命令式代码"的分离确保了配置系统的可预测性和安全性。
...
SDK 核心类型模块定义了完整的 `HOOK_EVENTS` 数组，共 31 个生命周期事件，涵盖工具调用、用户交互、会话管理、子代理、压缩、权限、配置变更等所有关键节点。
...
以下代码实现了第 9 章的核心概念——五种 Hook 类型、事件匹配、优先级链。复制到 `mini-hooks.ts` 后用 `npx tsx mini-hooks.ts` 运行。

> **源码参考：** 对应 Claude Code `src/hooks/` 目录中的钩子执行逻辑和 `src/hooks/useCanUseTool.tsx` 中的权限钩子集成。

```typescript
// mini-hooks.ts — 最小钩子系统（~60 行）
// 源码参考：Claude Code src/hooks/

import { execSync } from "child_process";

type HookEvent = "PreToolUse" | "PostToolUse" | "Stop" | "Notification" | "SubagentStop";
interface HookDef { event: HookEvent; command: string; matcher?: string; }
interface HookResult { decision?: "allow" | "deny"; additionalContext?: string; }
...
function executeHook(hook: HookDef, context: Record<string, any>): HookResult {
  if (hook.matcher && context.toolName && !matchGlob(hook.matcher, context.toolName)) return {};
  try {
    const env = { ...process.env, CLAUDE_CONTEXT: JSON.stringify(context) };
    const output = execSync(hook.command, { encoding: "utf-8", timeout: 5000, env }).trim();
    try { return JSON.parse(output); } catch { return { additionalContext: output }; }
  } catch { return { decision: "deny", additionalContext: "Hook blocked" }; }
}

function main() {
  console.log("=== 钩子系统测试 ===\n");
  const hooks: HookDef[] = [
    { event: "PreToolUse", command: 'echo \'{"decision":"allow"}\'', matcher: "read_file" },
    { event: "PreToolUse", command: 'echo \'{"decision":"deny","additionalContext":"Blocked"}\'', matcher: "bash" },
  ];

  console.log("1. PreToolUse read_file → allow:");
  const r1 = executeHook(hooks[0], { toolName: "read_file" });
  console.log(`  decision: ${r1.decision}\n`);

  console.log("2. PreToolUse bash → deny:");
  const r2 = executeHook(hooks[1], { toolName: "bash" });
  console.log(`  decision: ${r2.decision}, reason: ${r2.additionalContext}\n`);

  console.log("3. Matcher skip (bash hook on read_file):");
  const r3 = executeHook(hooks[1], { toolName: "read_file" });
  console.log(`  decision: ${r3.decision || "(skipped)"}\n`);

  console.log("4. Priority chain (deny wins over allow):");
  const results = hooks.map(h => executeHook(h, { toolName: "bash" }));
  const final = results.find(r => r.decision === "deny")?.decision || "allow";
...
2. **31 个生命周期事件**：覆盖工具调用、用户交互、会话管理、子代理、压缩、权限、配置变更等完整的 Agent 生命周期。每个事件都有明确的触发时机、输入结构和退出码语义。
