...
| | **可迁移的认知模型** | 无论你用 LangChain、AutoGen、CrewAI 还是从零构建，书中 168 张架构图直接复用 |
...
> **时间紧张？** 00 → 01 → 02 → 04 → 19，拿到核心认知和动手能力就够用
>
> **有经验？** 直接读 Part 2 + Part 3，遇到概念缺口回溯 Part 1
>
> **系统学习？** 从头到尾，每章做练习，最后 Ch17 构建自己的 Harness（约 2–3 周）
>
> **想动手写代码？** 每章的 `code/` 子目录包含可运行的 Rust 实现，阅读完文档后直接 `cargo run` 上手
>
...
| # | 章节 | 核心内容 |
|:-:|------|---------|
| 00 | [预备知识](第一部分-基础篇/00-预备知识.md) | TypeScript 核心速修；异步编程三板斧（Promise→async/await→AsyncGenerator）；Zod 运行时验证；React/Ink 终端 UI；HTTP/SSE/JSON-RPC 通信基础；LLM API 调用基础 |
...
| 03 | [工具系统 — Agent 的双手](第一部分-基础篇/03-工具系统-Agent的双手.md) | `Tool<I,O,P>` 五要素协议；`buildTool` 故障安全工厂；66+ 工具 × 12 类；并发分区贪心算法 |
| 04 | [权限管线 — Agent 的护栏](第一部分-基础篇/04-权限管线-Agent的护栏.md) | 七层纵深防御；五种外部权限模式；Bash AST + 23 项静态检查；推测性分类器 2 秒 Promise.race |

### Part 2. 核心系统篇 — 深入子系统

> 拆解 Agent Harness 的五大核心子系统——配置、记忆、上下文、错误恢复、钩子。
...
| 07 | [上下文管理 — Agent 的工作记忆](第二部分-核心系统篇/07-上下文管理-Agent的工作记忆.md) | 有效窗口公式；五级渐进压缩（Tool Result 预算→Snip→MicroCompact→Collapse→AutoCompact）；断路器模式 |
| 08 | [错误恢复 — Agent 的韧性](第二部分-核心系统篇/08-错误恢复-Agent的韧性.md) | `prompt_too_long` 分层恢复；`max_output_tokens` 延续优先；响应式压缩防死循环；断路器模式；中断账本闭合 |
| 09 | [钩子系统 — Agent 的生命周期扩展点](第二部分-核心系统篇/09-钩子系统-Agent的生命周期扩展点.md) | 五种 Hook 类型；31 个生命周期事件；JSON 响应协议；六层优先级；三层安全机制 |

### Part 3. 高级模式篇 — Agent 的组合与扩展

> 探索 Agent 如何组合、编排和扩展——从子智能体到 MCP 协议桥接，再到双系统架构对比。

| # | 章节 | 核心内容 |
|:-:|------|---------|
| 10 | [子智能体与 Fork 模式](第三部分-高级模式篇/10-子智能体与Fork模式.md) | 三种 Agent 来源；四种内置 Agent；Fork 字节级上下文继承；递归 Fork 防护 |
| 11 | [协调器模式 — 多智能体编排](第三部分-高级模式篇/11-协调器模式-多智能体编排.md) | Coordinator-Worker 双重门控；"只编排不执行"约束；四种寻址模式；四阶段工作流 |
| 12 | [技能系统与插件架构](第三部分-高级模式篇/12-技能系统与插件架构.md) | 11 个核心技能；SKILL.md frontmatter；三级参数替换；分层加载；插件缓存 |
| 13 | [MCP 集成与外部协议](第三部分-高级模式篇/13-MCP集成与外部协议.md) | 8 种传输协议；五态连接管理；三段式工具命名；Bridge 双向通信系统 |
| 14 | [Claude Code 与 Codex 的架构对比](第三部分-高级模式篇/14-Claude-Code与Codex的架构对比.md) | 控制平面：动态组装 vs 结构化片段；查询循环 vs Thread/Rollout/State；运行时编排 vs 策略语言；收敛与分叉 |

### Part 4. 工程实践篇 — 从原理到构建

> 性能优化的工程细节，团队落地指南，原则总结，以及从零构建一个完整 Harness 的实战路线图。

| # | 章节 | 核心内容 |
|:-:|------|---------|
| 15 | [流式架构与性能优化](第四部分-工程实践篇/15-流式架构与性能优化.md) | QueryEngine 生命周期管理；并发控制；启动优化 160ms→65ms（-59%）；惰性加载策略 |
| 16 | [Plan 模式与结构化工作流](第四部分-工程实践篇/16-Plan模式与结构化工作流.md) | "先想后做"哲学；计划文件三层恢复策略；本地调度与远程触发 |
| 17 | [构建你自己的 Agent Harness](第四部分-工程实践篇/17-构建你自己的Agent-Harness.md) | 六步实现路线图；循环依赖解决方案；四层可观测性体系；安全威胁模型 |
| 18 | [团队采纳 — 从工具到制度](第四部分-工程实践篇/18-团队采纳-从工具到制度.md) | 三类团队画像；分阶段检查清单；CLAUDE.md vs AGENTS.md；失败/成功编码机制 |
| 19 | [Agent Harness 十大原则](第四部分-工程实践篇/19-Agent-Harness十大原则.md) | 约束执行；Prompt 是控制平面；错误是主路径；调度纪律；分层恢复；上下文预算；制度比聪明可靠 |
...
| [E](附录/E-工程检查清单.md) | **工程检查清单** — 运行前检查、设计评审、上下文治理、安全加固、生产部署五套清单 |
